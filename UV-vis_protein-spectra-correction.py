# -*- coding: utf-8 -*-
"""
Created on Fri Apr 29 16:55:54 2022
...
def fct_baseline(x, a, b):
    return(a/np.power(x,4)+b)   #scattering correction : if you need a ref for this, use https://doi.org/10.1107/S1399004714015107 
...
def baselinefitcorr_3seg_smooth(df,  segment1, segment2, segmentend, sigmaby3segment):
    ...
    print(sigma)
    para, pcov = sp.optimize.curve_fit(f=fct_baseline, xdata=x, ydata=y, p0=initialParameters, sigma=sigma)  #used to fit the 1/lamda^4 correction
    baseline=df.copy()
...
#here is the snippet of code to read and correct spectra
#I left the commented plot blocks of code for diagnosis
...
directory = "./"  
raw_spec={}                #Dictionnary which will contain raw data
ready_spec={}              #Dictionnary which will contain data corrected for scattering and scaled
ready_spec_nolam4={}       #Dictionnary which will contain data corrected only with the substraction of a constant taken around 800nm where the spectrum is flat
for entry in os.scandir(directory):   # Parsing over all files in the home directory
    if entry.path.endswith(".txt") and entry.is_file() :   #I save spectra in txt format (tab delimited) and I keep the header because it contains usefull metadata
        a=entry.path[2:-4]                               #This is to keep only the name, in my case, I have 2 characters of path (./) before the name of your file, and 4 char which make up the extension
        print(a)
        tmp=pd.read_csv(filepath_or_buffer= entry.path,  
                        sep= "\t",                       
                        decimal=".",                
                        skiprows=17,                      #There are 17 rows of header
...
#        plt.show()
        ...
        #First segment of the spectrum used to fit the 1/lamda^4 correction : near the point where teh scattering is maximised before we stop getting photons
        ...
        #2nd segment of the spectrum used to fit the 1/lamda^4 correction : right after the peak for 280nm
        ...
        #3rd segment of the spectrum used to fit the 1/lamda^4 correction : I take my baseline segment where the spectrum is flat, you might want to play around with this depending on the absorption peak of your samples
        leftborn=closest(tmp.wl,700)
        rightborn=closest(tmp.wl,800)
        ...
        # plt.plot(tmp.wl, tmp.origin)
        # plt.show()
        sigmafor3segment=[1,0.1,0.1]    # The variance allowed for the fitting of the model. it is greater on the end of the spectrum because the data there starts being a bit spurrious (less photons)
